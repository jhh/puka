# type: ignore
# ruff: noqa: F821 INP001 PGH003 S101 S105 S108 S311
import random
import string
...
with subtest("check static files"):
    machine.succeed(
        f"wget --no-verbose --load-cookies {cookie_jar_path} --level=1 --spider --recursive {base_url}",
    )
...
with subtest("create a new item"):
    ...
    output = machine.succeed(f"{curl} --location {base_url}/stuff/item/1/")
